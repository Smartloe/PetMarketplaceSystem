...
### Codex & DeepSeek Integration Notes

- When using Codex or DeepSeek models for code generation or refactoring tasks, ensure that **all shell commands are executed through the defined shell agent functions**, not directly from the model output.
- Do **not** allow models to run arbitrary system commands directly.
- If the model needs to perform file operations (create, modify, or delete files), ensure those actions are executed via controlled functions (e.g., `run_shell()`, `safe_write()`) instead of inline code.
- The model configuration profile for DeepSeek is defined in `~/.codex/config.toml` under `[model_providers.deepseek-chat]` and `[model_providers.deepseek-reasoner]`.
- All AI-related API keys (e.g., `DEEPSEEK_API_KEY`, `LONGCAT_API_KEY`) must be set via environment variables or `.env` files — never hardcoded in source code.
